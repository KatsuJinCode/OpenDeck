...
pub async fn register_device(uuid: &str, mut event: PayloadEvent<crate::shared::DeviceInfo>) -> Result<(), anyhow::Error> {
	if uuid.is_empty() || Some(uuid) == DEVICE_NAMESPACES.read().await.get(&event.payload.id[..2]).map(|x| x.as_str()) {
		...
		// Lazy plugin activation on device connect: ensure plugins referenced
		// by this device's active profile are spawned before firing will_appear.
...
/// Handle a touch-strip swipe gesture. The HID layer provides start and end
/// (x, y) coordinates. We convert x to encoder position (each encoder owns a
/// 200px-wide strip), require the swipe to span at least 3 encoder positions,
/// and cycle to the next/previous profile alphabetically.
pub async fn touch_swipe(device: String, from: (u16, u16), to: (u16, u16)) -> Result<(), anyhow::Error> {
	...
	// old slots, clears the device screen, fires willAppear for new slots,
	// and saves the selection. This is the same path the frontend uses.
